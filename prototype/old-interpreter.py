import fileinput
# Chunking tools to translate groups of verbs/nouns into Subject-Verb-Object
import nltk.chunk, nltk.chunk.util, nltk.chunk.regexp
...
for paragraph in fileinput.input():
    ...
    for sentence in [nltk.tag.pos_tag(sentence) for sentence in sentences]:
        ...
        chunk_rules = [
            # Imperative clauses (subject is implied -- usually the interpreter)
            "<VB.?><CD>(?!<NN.?>)", # Get verbs followed by a number that *isn't* counting objects (e.g. "add seven")
            ""
        ]
        # Get chunked result
